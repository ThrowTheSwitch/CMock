...
class CMockGenerator
  ...
  def create_mock(module_name, parsed_stuff, ext)
    @module_ext = ext
  ...
  def create_mock_header_file(parsed_stuff)
    if @include_inline == :include
      @file_writer.create_file(@module_name + @module_ext, @subdir) do |file, filename|
        file << parsed_stuff[:normalized_source]
      end
    end

    @file_writer.create_file(@mock_name + @module_ext, @subdir) do |file, filename|
  ...
  def create_mock_header_header(file, filename)
    ...
    orig_filename = (@subdir ? @subdir + "/" : "") + @module_name + @module_ext
  ...
  def create_source_header_section(file, filename, functions)
    header_file = (@subdir ? @subdir + '/' : '') + filename.gsub(".c",@module_ext)
    file << "/* AUTOGENERATED FILE. DO NOT EDIT. */\n"
