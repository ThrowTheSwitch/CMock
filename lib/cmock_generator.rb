...
class CMockGenerator
  ...
  def create_mock(module_name, defnfile, parsed_stuff, target_folder, initialiseFunctions)
    @module_name = module_name
    @mock_name   = @prefix + @module_name
    create_mock_header_file(parsed_stuff, defnfile, target_folder, initialiseFunctions)
  ...
  def create_mock_header_file(parsed_stuff, defnfile, target_folder, initialiseFunctions)
    @file_writer.create_file(@mock_name + ".h", target_folder) do |file, filename|
      create_mock_header_header(file, filename, defnfile, parsed_stuff[:includes])
      create_mock_header_service_call_declarations(file, initialiseFunctions)
  ...
  def create_mock_header_service_call_declarations(file, initialiseFunctions) 
